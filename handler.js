...
//ett pendaftar = JSON.parse(fs.readFileSync('./lib/json/pengguna.json'))
const afkJs = require('./lib/afk')
const moment = require('moment-timezone');
const { mess, menu, ingfo, sdk, donasi, listCode } = require('./lib/text')
...
module.exports = handle = (client, Client, baterai, pendaftar, pengguna) => {
    try {
        /*DOWNLOADER*/
        ...
        Client.cmd.on('tiktokaudio', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}tiktokaudio [ link ]*\nContoh : ${data.prefix}tiktokaudio https://vt.tiktok.com/ZSJTYpE7A/`)
                data.reply(mess.wait)
                res = await axios.get(`https://dhnjing.xyz/downloader/tiktok/music?url=${data.body}&apikey=ExnephZyy`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data.result
                teks = `*Data berhasil didapatkan!*\n\nTittle: ${ytm.title}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                Client.sendFileFromUrl(data.from, `${ytm.coverThumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.playUrl}`, `${ytm.title} - Download.mp3`, ``, data.message)
            } catch {
                data.reply('Ups maaf server sedang error atau mungkin apikey invalid')
            }
        })
        ...
        Client.cmd.on('play', async (data) => {
            try {
                ...
                data.reply('Ups maaf sepertinya ada yang error')
            }
        })
        Client.cmd.on('ig', async (data) => {
            ...
            data.reply(mess.wait)
            getresult = await axios.get(`https://megayaa.herokuapp.com/api/igdl?url=${data.body}`)
        ...
        Client.cmd.on('tiktoknowm', async (data) => {
          try {
          if(isLimit(data.sender)) return data.reply(mess.limit)
          if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}tiktoknowm [ link ]*\nContoh : ${data.prefix}tiktoknowm https://vt.tiktok.com/ZSJTYpE7A/`)
          data.reply(mess.wait)
          r = await axios.get(`https://megayaa.herokuapp.com/api/tiktod/?url=${data.body}`)
          const mediaMsg = await getBuffer(r.data.result.nowatermark)
          Client.sendFileFromUrl(data.from, `${r.data.result.nowatermark}`, 'video.mp4', `Video telah terkirim @${data.sender.split('@')[0]}`, data.message)
          } catch {
          data.reply('Pastikan link valid')
        }
        })
        Client.cmd.on('tiktokwm', async (data) => {
          try {
          if(isLimit(data.sender)) return data.reply(mess.limit)
          if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}tiktokwm [ link ]*\nContoh : ${data.prefix}tiktoknowm https://vt.tiktok.com/ZSJTYpE7A/`)
          data.reply(mess.wait)
          r = await axios.get(`https://megayaa.herokuapp.com/api/tiktod/?url=${data.body}`)
          const mediaMsg = await getBuffer(r.data.result.watermark)
          Client.sendFileFromUrl(data.from, `${r.data.result.nowatermark}`, 'video.mp4', `Video telah terkirim @${data.sender.split('@')[0]}`, data.message)
        } catch {
          data.reply('Pastikan link valid')
        }
        ...
        Client.cmd.on('igstory', async (data) => {
            try {
                ...
                data.reply(mess.wait)
                stomr = await axios.get(`https://megayaa.herokuapp.com/api/igstori?username=${data.body}`)
        ...
        Client.cmd.on('soundcloud', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}soundcloud [ link ]*\nContoh : ${data.prefix}soundcloud https://soundcloud.com/aviwkila/aviwkila-doa-untuk-kamu`)
                data.reply(mess.wait)
                res = await axios.get(`https://kuhong-api.herokuapp.com/api/soundcloud?url=${data.body}&apikey=MCIlR`)
                jox = res.data.result
                teks = `*Data berhasil didapatkan!*\n\n*Judul* : ${jox.title}\n*Duration* : ${jox.duration}\n*Quality* : ${jox.quality}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                Client.sendFileFromUrl(data.from, `${jox.thumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${jox.download}`, 'audio.mp3', ``, data.message)
            } catch {
                data.reply('Maaf lagu tidak ditemukan')
            }
        })
       ...
       Client.cmd.on('fakta', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            res = await axios.get(`https://dhnjing.xyz/random/fakta/v1`)
            data.reply('Ｆａｋｔａ\n\n*'+res.data.result+'*')
        })
        Client.cmd.on('katabijak', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            res = await axios.get(`https://dhnjing.xyz/random/katabijak/v1`)
            data.reply('Ｋａｔａ　Ｂｉｊａｋ\n\n*'+res.data.result+'*')
        })
        Client.cmd.on('covidworld', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            res = await axios.get(`https://megayaa.herokuapp.com/api/covidindo`)
            data.reply('Ｃｏｖｉｄ　Ｗｏｒｌｄ\n\nTotal Cases : *'+res.totalcases+'*\n*Recovered : '+res.data.Recovered+'*\n *Deaths : '+res.data.deaths)
        })
            ...
            const buttonMessage = {
				  ...
				  footerText: 'Press the button below to get a random waifu image',
                        "contextInfo": {
                              ...
                              quotedMessage: data.message.message, thumbnail:Buffer.alloc(0),
							  },
                              buttons: [
                                {
        ...
        Client.cmd.on('tebakan', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.tebakan[data.from] && global.tebakan[data.from].id) return data.reply("Masih ada soal yang berjalan")
            const getSoal = await axios.get(`https://dhnjing.xyz/fun/tebakan`)
			ses = Date.now()
			send = await Client.reply(data.from, `Pertanyaan : ${getSoal.data.result.pertanyaan} \n\nWaktu menjawab 30 detik!`)
			global.tebakan[data.from] = {jawaban: getSoal.data.result.jawaban, id: ses}
			await sleep(10000)
			if (global.tebakan[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 20 detik", send)
			await sleep(10000)
			if (global.tebakan[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 10 detik", send)
			...
			Client.reply(data.from, "Waktu habis", send)
			Client.reply(data.from,`Jawabannya adalah: ${getSoal.data.result.jawaban}`, send)
			global.tebakan[data.from] = {}
			
        })
        Client.cmd.on('family100', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.family100[data.from] && global.family100[data.from].id) return data.reply("Masih ada soal yang berjalan")
            const getSoal = await axios.get(`https://dhnjing.xyz/fun/tebakan`)
			ses = Date.now()
			send = await Client.reply(data.from, `Pertanyaan : ${getSoal.data.result.pertanyaan} \n\nWaktu menjawab 30 detik!`)
			global.family100[data.from] = {jawaban: getSoal.data.result.jawaban, id: ses}
			await sleep(10000)
			if (global.family100[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 20 detik", send)
			await sleep(10000)
			if (global.family100[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 10 detik", send)
			...
			Client.reply(data.from, "Waktu habis", send)
			Client.reply(data.from,`Jawabannya adalah: ${getSoal.data.result.jawaban}`, send)
			global.family100[data.from] = {}
			
        })
        Client.cmd.on('siapakahaku', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.siapakahaku[data.from] && global.siapakahaku[data.from].id) return data.reply("Masih ada soal yang berjalan")
            const getSoal = await axios.get(`https://dhnjing.xyz/fun/siapakahaku`)
			ses = Date.now()
			send = await Client.reply(data.from, `Pertanyaan : ${getSoal.data.result.pertanyaan} \n\nWaktu menjawab 30 detik!`)
			global.siapakahaku[data.from] = {jawaban: getSoal.data.result.jawaban, id: ses}
			await sleep(10000)
			if (global.siapakahaku[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 20 detik", send)
			await sleep(10000)
			if (global.siapakahaku[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 10 detik", send)
			...
			Client.reply(data.from, "Waktu habis", send)
			Client.reply(data.from,`Jawabannya adalah: ${getSoal.data.result.jawaban}`, send)
			global.siapakahaku[data.from] = {}
			
        })
        Client.cmd.on('tebakkota', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.tebakkota[data.from] && global.tebakkota[data.from].id) return data.reply("Masih ada soal yang berjalan")
            const getSoal = await axios.get(`https://dhnjing.xyz/fun/tebakkota`)
			ses = Date.now()
			send = await Client.reply(data.from, `Pertanyaan : ${getSoal.data.result.soal} \n\nWaktu menjawab 30 detik!`)
			global.tebakkota[data.from] = {jawaban: getSoal.data.result.jawaban, id: ses}
			await sleep(10000)
			if (global.tebakkota[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 20 detik", send)
			await sleep(10000)
			if (global.tebakkota[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 10 detik", send)
        ...
        Client.cmd.on('bc', async (data) => {
            ...
            list.forEach(async dataC => {
                ...
                else Client.sendText(dataC.jid, `*ｅｘｎｅｐｈ　ｂｏｔ　ｂｒｏａｄｃａｓｔ*\n${data.body}\n\n`)
            })
        })
        ...
        Client.cmd.on('owner', async (data) => {
            Client.sendContact(data.from, { number: configs.ownerList[0].split('@')[0], name: 'Azyansah | Exneph Bot Owner' }, data.message)
        })
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
                ...
                case 'list':
                     ...
                     const buttonMessage = {
                           contentText: menu(data.prefix, data.pushname, data.totalchat, data.giid),
                           footerText: ' _Made with <3 by azyansah_',
                                "contextInfo": {
									  mentionedJid: [configs.ownerList[0]],
                                      participant: sender,
                                      stanzaId: message.key.id,
                                      quotedMessage: message.message, thumbnail:Buffer.alloc(0),
                                     },
                                     buttons: [
                                     {
                                       buttonId: `${data.prefix}info`,
                                       buttonText: {
                                          displayText: "Ｉｎｆｏｒｍａｔｉｏｎ"
                                        },
                                         "type": "RESPONSE"
                                     },
                                      {
                                       ...
                                       buttonText: {
                                          displayText: "Ｄｏｎａｔｉｏｎｓ"
                                        },
                                         "type": "RESPONSE"
                                     },
                                     {
                                       buttonId: `${data.prefix}sdk`,
                                       buttonText: {
                                          displayText: "Ｔｅｒｍｓ　＆　Ｃｏｎｄｉｔｉｏｎｓ"
                                        },
                                         "type": "RESPONSE"
                                     },
                                        ],
...
case 'botstat':{
                ...
                let timestampi1 = speed();
				let latensii1 = speed() - timestampi1
                const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = client.user.phone
                let anu = process.uptime()
                let teskny = `　Ｅｘｎｅｐｈ　Ｂｏｔ　Ｓｔａｔｉｓｔｉｃ　

*Versi Whatsappp :* ${wa_version}
...
*Speed :* ${latensii1.toFixed(4)} Second`
				data.reply(teskny)
            }
            break
case 'speed':
                    const timestamp = speed();
                    const latensi = speed() - timestamp
                    data.reply(`Speed: ${latensi.toFixed(4)} Second`)
                    break
//Fun
case 'truth':
			if(isLimit(data.sender)) return data.reply(mess.limit)
			const trut =['Pernah suka sama siapa aja? berapa lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?','Kapan lo terakhir kali kesetrum?🤔']
...
if (isLimit(data.sender)) return data.reply(mess.limit)
   ifakah = data.body
if (!ifakah) return data.reply(`Kirim Perintah ${data.prefix}apakah (query)!`)
					const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi']
					const kah = apa[Math.floor(Math.random() * apa.length)]
          data.reply('Pertanyaan : Apakah *'+ifakah+'*\n\nJawaban : '+ kah)
break
case 'bisakah':
if (isLimit(data.sender)) return data.reply(mess.limit)
   ifakah = data.body
if (!ifakah) return data.reply(`Kirim Perintah ${data.prefix}bisakah (query)!`)
					const bisa =['Gabisa','Bisa','Bisa Jadi','Mungkin','Coba Aja Manatau Bisa','YNTKTS','Bisa Banget']
					const kh = bisa[Math.floor(Math.random() * bisa.length)]
          data.reply('Pertanyaan : Bisakah *'+ifakah+'*\n\nJawaban : '+ kh)
break
case 'kapankah':
if (isLimit(data.sender)) return data.reply(mess.limit)
kapankah = data.body
if (!kapankah) return data.reply(`Kirim Perintah ${data.prefix}kapankah (query)!`)
					const kapan =['Besok','Lusa','Sebentar Lagi','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
					const koh = kapan[Math.floor(Math.random() * kapan.length)]
					data.reply('Pertanyaan : Kapankah *'+kapankah+'*\n\nJawaban : '+ koh)
					break
                    /*STICKER*/
                case 'sgif':
                case 'sticker':
               ...
               case 'ipemoji':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ emoji ]*\nContoh : ${data.prefix}${data.command} 😃`)
                        Client.sendStickerFromUrl(from, `https://kuhong-api.herokuapp.com/api/emojitopng?emoji=${encodeURIComponent(data.body)}&apikey=MCIlR`, message, { emojis: data.body.match(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g)})
                    } catch {
                        data.reply('error')
                    }
                    break
...
/*                    case "soundcloud":
      case "scdl":
        if(isLimit(data.sender)) return data.reply(mess.limit)
          ...
          await Client.sendFileFromUrl({url: a.data.image}, title)
          await Client.sendFileFromUrl({url: result})
        break*/
                case 'playstore':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                ...
                case 'igstalk':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}igstalk [ query ]*\nContoh : ${data.prefix}igstalk creallizy.xd`)
                        data.reply(mess.wait)
                        res = await axios.get(`https://dhnjing.xyz/stalk/instagram?user=${data.body}&apikey=ExnephZyy`)
                        pe = res.data.result
                        tek = `*「 INSTAGRAM PROFILE 」*\n*Username:* @${pe.username}\n*Nama:* ${pe.user_fullname}\n*Pengikut:* ${pe.user_followers}\n*Mengikuti*: ${pe.user_following}\n*Deskripsi:* ${pe.user_bio}\n*Total Post:* ${pe.user_post_total}\n*Link:* https://instagram.com/${pe.username}
`
                        Client.sendFileFromUrl(from, pe.user_profile_hd, 'p.jpg', tek, message)
                    } catch {
                        data.reply(`Maaf username ${data.body} tidak ditemukan`)
                    }
                    break
...
*Speed :* ${latensii1.toFixed(4)} Second`
            ...
            break
              case '1.2937335358575338e+123':
                    const timestamp = speed();
                    const latensi = speed() - timestamp
                    datas.reply(`Kecepatan Bot\n\n${latensi.toFixed(4)} Second`)
                    break
                case '2.453746655066493e+123':
                    datas.reply(menu(configs.prefix == 'multi' ? '.' : configs.prefix, pushname, pendaftar))
                    break
