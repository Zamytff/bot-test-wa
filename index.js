...
let pengguna = JSON.parse(fs.readFileSync('./lib/json/pengguna.json'))
...
const starts = async (sesName) => {
   ...
   baterai = `${batterylevel}`
        ...
        client.on('new-msg', (message) => {
			...
			if (global.family100[from] && global.family100[from].id && global.family100[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `YES FAMILY 100 BERHASIL DIJAWAB OLEH @${sender.split("@")[0]}`, message).then(() => global.family100[from] = {})
			if (global.tebakan[from] && global.tebakan[from].id && global.tebakan[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `YES TEBAKAN BERHASIL DIJAWAB OLEH @${sender.split("@")[0]}`, message).then(() => global.tebakan[from] = {})
			if (global.siapakahaku[from] && global.siapakahaku[from].id && global.siapakahaku[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `YES TEBAKAN SIAPAKAH AKU BERHASIL DIJAWAB OLEH @${sender.split("@")[0]}`, message).then(() => global.siapakahaku[from] = {})
			if (global.tebakkota[from] && global.tebakkota[from].id && global.tebakkota[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `YES TEBAK KOTA BERHASIL DIJAWAB OLEH @${sender.split("@")[0]}`, message).then(() => global.tebakkota[from] = {})
			...
			if (!pengguna[sender]){
				pengguna[sender] = {}
				fs.writeFileSync('./lib/json/pengguna.json', JSON.stringify(dataUser))
			}
            if(isGroup) {
